...
void CreateBuildingButton::Load(rapidjson::Value & obj) {
	...
	if (obj.HasMember("BuildingArchetype") && (tmp = &obj["BuildingArchetype"])->IsString()) {
		...
		buildingArchetype = GetParent()->GetGameObjectManager()->GetArchetype(archetypeName.c_str());
	}
}
...
void CreateBuildingButton::InstanceInit() {
	if (buildingArchetype) {
		...
		SetActive(Building::IsUnlocked(army->GetSide(), buildingType) && army->GetFunds() >= Building::buildingCost[buildingType]);
	}
}
...
void CreateBuildingButton::Click(float dt) {
